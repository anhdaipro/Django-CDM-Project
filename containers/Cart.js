...
import {localhost,formatter,listThreadlURL,ItemRecommend,cartURL,itemvariation,rating,orderURL,
    updatecartURL,arraymove, savevoucherURL} from "../constants"
...
export default class Cart extends React.Component{
    constructor(props) {
    super(props);
    this.state = {list_item_recommend:[],loading_item:false,page_no:1,items:[],size_id:0,color_id:0,
    ...
    async componentDidMount(){
        axios.all([
            axios.get(cartURL,{ 'headers': { Authorization:`JWT ${localStorage.token}` }}),
            axios.get(orderURL,{ 'headers': { Authorization:`JWT ${localStorage.token}` }}),
        ])
    ...
    checkall(e){
        ...
        axios.post(cartURL,form,{ 'headers': { Authorization:`JWT ${localStorage.token}`}})
        .then(rep=>{
            let data=rep.data
    ...
    update(e,data,item){
        ...
        if(e.target.classList.contains('button-solid')){
            ...
            if(variation_active.length>=this.state.count_variation){
                ...
                axios.post(updatecartURL,form,{ 'headers': { Authorization:`JWT ${localStorage.token}`}})
                .then(rep=>{
                    let result=rep.data
    ...
    save_voucher(voucher,shop,list_shop){
        ...
        this.setState({[list_shop]:shop})
        axios.post(savevoucherURL,form,{ 'headers': { Authorization:`JWT ${localStorage.token}`}})
        .then(rep=>{
            let data=rep.data
        })
    }
        ...
        <div className="d-flex p-1">
             <div className="item-check item-center">
                ...
                <label className={`stardust-checkbox ${data.check?'stardust-checkbox--checked':''}`}>
                    <input defaultChecked onChange={(e)=>this.checked(e,data,item,index)} type="checkbox" value={data.id} checked={data.check?true:false} className="stardust-checkbox__input" type="checkbox"/>
    ...
    setshowthread=(order)=>{
        if(this.state.list_threads.length>0){
            if(this.state.list_threads.some(thread=>thread.list_participants.includes(this.state.user.user_id) && thread.list_participants.includes(order.shop_user))){
                            ...
                            <div className="_1nft6V">
                                ...
                                <div className="_2jol0L _3GVi82">
                                    ...
                                    <div className="j1liQm DOOM4N _2gEAGZ item-center">
                                        ...
                                        <div className="_1hOsmP">Shopee Xu</div>
                                        <div className="_1WYjnM">Bạn chưa có Shopee Xu</div>
